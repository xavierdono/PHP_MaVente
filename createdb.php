...
$query = "CREATE TABLE items (
            ...
            items_discount real,
            items_price real,
            items_date_publish date
          )";
$base->exec($query);

for ($x = 0; $x < 55; $x++) {
    $query = "INSERT INTO items (items_id, items_name_en, items_name_fr, items_desc_en, items_desc_fr, items_ean, fk_manu_id, fk_cat_id, fk_sta_id, fk_war_id, fk_trs_id, fk_inc_id, items_discount, items_price, items_date_publish) "
            . "VALUES ('" . $x . "', '"
            ...
            . strftime("%d/%m/%Y", strtotime('', $faker_fr->unixTime($max = 'now')))  . "')";
    $base->exec($query);
}
